/*
 * Base Framework for Augur. This is the main entry point for the back-end. This is either invoked by the  
 * administrators or by the GUI.
 */
package augur.org;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		if (args[0].equals("t")) {
			...
			if (args[0].equals("m")) {
				...
				// predefined steps. See StepFactory for list of predefined
				// steps
				...
				// list of
				// arguments
				StepConfig customStep = new StepConfig("augurTrain",
						hadoopConfig1);

				RunJobFlowResult result = client
								...
								.withMasterInstanceType(
										...
										.withSlaveInstanceType(
												...
												.withKeepJobFlowAliveWhenNoSteps(
														...
														.withName("augurmr")
														.withLogUri("s3://augurframework/log/")
				 ...
				 * 
				 * 1. If movie is in DB, load the already calculated collection
				 * and display the result if rePrediction is not requested.
				 * ELse, go to step 2. 2. Load data for the movie from various
				 * sources and write to S3. 3. Run EMR on the data from S3. 4.
				 * Aggregate the data, and load it in Hive DB, along with the
				 * prediction info. Display the prediction.
				 */
				String[] argRun = new String[args.length - 1];
	...
	private static void runDemo(String[] args) {
		if (args.length > 2 || args.length < 1) {
			System.out
			.println("Usage: java -jar <jarName> <movieName> [clean]");
			return;
		}
		...
		if (args.length == 2) {
			for (S3ObjectSummary mrOutFile : s3.listObjects("augurframework",
					"mapreduceOu").getObjectSummaries()) {
		...
		//
		// client.addJobFlowSteps(new
		// AddJobFlowStepsRequest().withSteps(customStep).withJobFlowId(result.getJobFlowId()));
		//
		// System.out.println(result.getJobFlowId());

		System.out.println("EMR JobFlowId - j-2LY0MGXX4E8LY");

		client.addJobFlowSteps(new AddJobFlowStepsRequest().withJobFlowId(
				"j-2LY0MGXX4E8LY").withSteps(customStep));
		...
		.withBucketName("augurframework").withPrefix(
				"mapreduceOutput/part");
		...
		s3.getObject(new GetObjectRequest("augurframework", "bin/DB.jar"),
				dbJarFile);
		...
		s3.getObject(
				new GetObjectRequest("augurframework", "bin/Starcast.jar"),
				starcastFile);

		listObjectsRequest = new ListObjectsRequest().withBucketName(
				"augurframework").withPrefix("cast/");
		...
		.withBucketName("augurframework").withPrefix("starmap/");
		...
		do {
			...
			for (S3ObjectSummary objectSummary : objectListing
				...
				if (objectSummary.getSize() > 0) {
					s3.getObject(new GetObjectRequest("augurframework",
							objectSummary.getKey()), starMapFile);
				}
				System.out.println(" - " + objectSummary.getKey() + "  "
						+ "(size = " + objectSummary.getSize() + ")");
			}
		...
		String starpowerCmd = "java -jar Starcast.jar " + filePathRoot
				+ "cast " + filePathRoot + "starmap";
		if (runProcess(starpowerCmd) == 1) {
			return;
		}

		// Load starpower in DB
		String starpowerloadCmd = "java -jar DB.jar starpower " + filePathRoot
				+ "starpower.txt";
		...
		s3.getObject(
				new GetObjectRequest("augurframework", "bin/Predictor.jar"),
				predJarFile);
